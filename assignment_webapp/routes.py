...
def index():
    ...
    # if database.is_superuser(user_details['username']) != None:
    if user_details['issuper']:
...
def submit():
    ...
    if(request.method == 'POST'):
        ...
        registerdata = (username,password,any_selected)
        database.checkregister(registerdata)
...
def single_podcastep(media_id):
    ...
    page['title'] = 'Media ID: '+media_id # Add the title
...
def single_genre(genre_id):
    ...
    # NOTE :: YOU WILL NEED TO MODIFY THIS TO PASS THE APPROPRIATE VARIABLES
    m = database.get_genre_movies_and_shows(genre_id)
    s = database.get_genre_songs(genre_id)
    p = database.get_genre_podcasts(genre_id)
    if m == None:
        m = []
    if s == None:
        s = []
    ...
    items = []
    if len(m) == len(p)==len(s)==0:
        page['title'] = ''

    elif len(m)>=len(p):
        if len(m)>=len(s):
            items = m
            page['title'] = 'genre movies and tv shows'
        else:
            items = s
            page['title'] = 'song genre'
    else:
        if len(p)>=len(s):
            items = p
            page['title'] = 'podcast genre'
        else:
            items = s
            page['title'] = 'song genre'


    #test_item = {'item_id': "420", 'item_title': "it's a title",'item_type': genre_id}
...
def add_song():
    ...
    if request.method == 'POST':
        # Set up some variables to manage the post returns
        if ('description' not in request.form):
            newdict['description'] = 'Empty description value'
        else:
            newdict['description'] = request.form['description']
            print("We have a value: ", newdict['description'])
        ...
        if ('storage_location' not in request.form):
            newdict['storage_location'] = 'Empty storage value'
        else:
            newdict['storage_location'] = request.form['storage_location']
            print("We have a value: ", newdict['storage_location'])

        if ('genre' not in request.form):
            newdict['genre'] = 'Empty genre value'
        else:
        ...
        songs = database.add_song_to_db(newdict['storage_location'],newdict['description'],newdict['song_title'],newdict['length'],newdict['genre'],newdict['artist_id'])
